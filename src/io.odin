...
io_set_base_path :: #force_inline proc(exe_path: string) {
	g_io_base_path = exe_path
}

io_resolve :: fp.join
io_resolve_static :: #force_inline proc(path: string, alloc := context.allocator) -> (string) {
	return io_resolve({ g_io_base_path, STATIC_DIR, path }, alloc)
}
...
io_ensure_dir :: #force_inline proc(path: string) {
	...
	log(.Debug, fmt.tprintln("Made directory with Errno", e_code))
}
...
// Things to save
Uprefs :: struct { // @FullyImmediateData
	canon_lang_key: [2]u8,
	...
	axis_mouse_movement_sensitivity: f32, // @Default(8.0)
	// NOTE: we can't have non-simulated mouse sensitivity really, it's better to leave it up to the OS
}
UPREFS_FILE :: "uprefs"
...
DEFAULT_UPREFS := Uprefs{
	...
	// Controls
	axis_mouse_movement_sensitivity = 8.0,
}